defmodule QuizworldRealtimeWeb.GameLive.Show do
  ...
  defp button_style(color) do
    "padding:1rem 1.1rem;border:none;border-radius:18px;background:#{color};color:white;font-size:1rem;font-weight:900;letter-spacing:-0.01em;cursor:pointer;box-shadow:0 12px 28px rgba(15,23,42,0.16);"
  end

  defp input_style do
    "padding:0.95rem 1rem;border:1px solid #e2e8f0;border-radius:16px;font-size:1rem;background:white;color:#0f172a;"
  end

  defp shell_style(snapshot) do
    background =
      cond do
        active?(snapshot) -> "radial-gradient(circle at top, rgba(124,58,237,0.24), transparent 34%), linear-gradient(180deg, #1f1147 0%, #312e81 42%, #f8f9fc 42.01%, #f8f9fc 100%)"
        reveal?(snapshot) -> "radial-gradient(circle at top, rgba(225,29,72,0.20), transparent 34%), linear-gradient(180deg, #3b1024 0%, #7f1d1d 42%, #fff7ed 42.01%, #fff7ed 100%)"
        finished?(snapshot) -> "radial-gradient(circle at top, rgba(37,99,235,0.20), transparent 34%), linear-gradient(180deg, #10213f 0%, #1d4ed8 42%, #eff6ff 42.01%, #eff6ff 100%)"
        true -> "radial-gradient(circle at top, rgba(5,150,105,0.18), transparent 34%), linear-gradient(180deg, #0f2230 0%, #134e4a 42%, #f5f7fb 42.01%, #f5f7fb 100%)"
      end
  ...
  defp hero_style do
    "position:relative;overflow:hidden;padding:1.85rem 1.85rem 1.65rem;border-radius:32px;background:linear-gradient(135deg,rgba(124,58,237,0.92),rgba(37,99,235,0.88));backdrop-filter:blur(18px);box-shadow:0 28px 80px rgba(49,46,129,0.24);border:1px solid rgba(255,255,255,0.14);"
  end

  defp hero_stat_card_style do
    "min-width:240px;padding:1rem 1.1rem;border-radius:24px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.16);backdrop-filter:blur(10px);"
  end

  defp eyebrow_style do
    "display:inline-flex;align-items:center;gap:0.5rem;padding:0.42rem 0.7rem;border-radius:999px;background:rgba(255,255,255,0.16);font-size:0.73rem;font-weight:900;letter-spacing:0.14em;text-transform:uppercase;color:#ede9fe;margin-bottom:0.9rem;"
  end
  ...
  defp status_pill_style(snapshot) do
    colors =
      cond do
        waiting?(snapshot) -> {"rgba(5,150,105,0.18)", "#d1fae5"}
        active?(snapshot) -> {"rgba(124,58,237,0.18)", "#ede9fe"}
        reveal?(snapshot) -> {"rgba(225,29,72,0.18)", "#ffe4e6"}
        finished?(snapshot) -> {"rgba(37,99,235,0.18)", "#dbeafe"}
  ...
  defp error_banner_style do
    "background:linear-gradient(145deg,#7f1d1d,#be123c);color:#ffe4e6;padding:1rem 1.15rem;border-radius:18px;margin-bottom:0.5rem;font-weight:800;box-shadow:0 18px 40px rgba(127,29,29,0.18);"
  end

  defp main_stage_style do
    "background:rgba(255,255,255,0.94);backdrop-filter:blur(12px);border:1px solid rgba(226,232,240,0.95);border-radius:32px;padding:1.35rem;box-shadow:0 24px 70px rgba(15,23,42,0.10);"
  end

  defp side_panel_style do
    "background:rgba(255,255,255,0.94);backdrop-filter:blur(12px);border:1px solid rgba(226,232,240,0.95);border-radius:28px;padding:1.15rem;box-shadow:0 18px 50px rgba(15,23,42,0.08);"
  end
  ...
  defp answer_button_style(index, enabled?, selected?) do
    palette =
      case rem(index, 4) do
        1 -> {"#fff1f2", "#fda4af", "#e11d48"}
        2 -> {"#eff6ff", "#93c5fd", "#2563eb"}
        3 -> {"#fffbeb", "#fbbf24", "#d97706"}
        _ -> {"#ecfdf5", "#86efac", "#059669"}
      end
    ...
    cond do
      selected? ->
        "padding:1rem 1.05rem;text-align:left;border-radius:22px;border:2px solid #{ink};background:#{bg};font-size:1rem;font-weight:850;cursor:default;color:#0f172a;display:flex;align-items:center;gap:0.95rem;box-shadow:0 14px 35px rgba(124,58,237,0.10);"
  ...
  defp answer_index_style(selected?) do
    if selected? do
      "width:2.4rem;height:2.4rem;border-radius:999px;display:grid;place-items:center;background:#7c3aed;color:white;font-weight:950;flex-shrink:0;"
    else
  ...
  defp reveal_answer_style(answer) do
    if answer["is_correct"] do
      "padding:1rem 1.05rem;border-radius:20px;border:2px solid #059669;background:linear-gradient(145deg,#dcfce7,#f0fdf4);display:flex;justify-content:space-between;align-items:center;gap:1rem;"
    else
  ...
  defp reveal_badge_style(answer) do
    if answer["is_correct"] do
      "padding:0.5rem 0.7rem;border-radius:999px;background:#059669;color:white;font-size:0.72rem;font-weight:900;letter-spacing:0.08em;text-transform:uppercase;"
    else
  ...
  defp leaderboard_row_style(index, current_player_id, player) do
    if player[:id] == current_player_id do
      "display:flex;justify-content:space-between;gap:0.85rem;background:linear-gradient(145deg,#ede9fe,#f5f3ff);padding:0.85rem;border-radius:18px;border:1px solid #c4b5fd;align-items:center;"
    else
      base =
        case index do
          1 -> "background:linear-gradient(145deg,#ede9fe,#f5f3ff);border:1px solid #c4b5fd;"
          2 -> "background:linear-gradient(145deg,#e2e8f0,#f8fafc);border:1px solid #cbd5e1;"
          3 -> "background:linear-gradient(145deg,#dbeafe,#eff6ff);border:1px solid #93c5fd;"
