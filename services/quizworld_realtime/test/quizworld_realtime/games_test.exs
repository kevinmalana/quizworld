defmodule QuizworldRealtime.GamesTest do
  ...
  test "creating a game with a host player returns separate player credentials" do
    pin = "H" <> Integer.to_string(System.unique_integer([:positive]))

    assert {:ok, snapshot, host_token, player_token, player_id} =
             Games.create_session(%{
               "pin" => pin,
               "host_id" => "host_test",
               "quiz_id" => "quiz_test",
               "questions" => [question()],
               "host_player" => %{"nickname" => "Kevin", "avatar" => "🎮"}
             })

    assert is_binary(host_token)
    assert is_binary(player_token)
    refute host_token == player_token

    assert %{id: ^player_id, nickname: "Kevin", avatar: "🎮"} =
             Enum.find(snapshot.players, &(&1.id == player_id))

    assert {:ok, active} = Games.start_game(pin, host_token)
    assert {:ok, _answered} = Games.submit_answer(pin, player_id, player_token, "a1", 0)
    assert active.status == "active"
  end

  test "invalid host-player input does not create a lobby" do
    pin = "X" <> Integer.to_string(System.unique_integer([:positive]))
    ...
    assert {:error, :invalid_player} =
             Games.create_session(%{
               ...
               "quiz_id" => "quiz_test",
               "questions" => [question()],
               "host_player" => "not-a-player"
             })

    assert {:error, :not_found} = Games.snapshot(pin)
  end
