...
test("catalog pages use stable keyset cursors instead of offsets", () => {
  ...
  assert.match(cursorHelpers, /count: "exact", head: true/);
  assert.match(study, /countQuery[\s\S]*head: true/);
  assert.match(cursorHelpers, /Promise\.all\(\[query, countQuery\]\)/);
  assert.match(study, /Promise\.all\(\[query, countQuery\]\)/);
  assert.match(explore, /fetchCatalogPage/);
  assert.match(cursorHelpers, /catalogCursorFilter/);
  assert.doesNotMatch(cursorHelpers, /\.range\(/);
