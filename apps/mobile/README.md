...
[Personal sync prerequisite and proposed contract](docs/personal-sync-gate.md) records the production metadata survey and narrowly scoped schema/policy decision. Cloud sync and a native official-completion bridge are **not enabled**: existing scoring writes XP/classroom completion and has no revision/offline-personal contract. Local history is never silently replayed to it. No migration or provider mutation was made.
