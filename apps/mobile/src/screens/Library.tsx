...
export function Library(){
 ...
 return <Page><Heading>Follow your curiosity.</Heading><Body muted>A new topic. A fresh perspective. What will you discover?</Body><Title>Offline little discoveries</Title>
 {samples.map(pack=><Card key={pack.id}><View style={s.row}><TopicArt topic={pack.category} size={76}/><View style={s.grow}><Meta>BUNDLED SAMPLE</Meta><Title>{pack.title}</Title></View></View><Body>{pack.questions.length} questions · flashcards or quickfire</Body><Button secondary label={`Open ${pack.title}`} onPress={()=>nav.navigate('Detail',{pack})}/></Card>)}
 ...
 {!items.length?<Card><Body>No matching public quizzes.</Body><Meta>Try a different title or category.</Meta></Card>:items.map(item=><Card key={item.id}><View style={s.row}><TopicArt topic={item.category} size={60}/><View style={s.grow}><Meta>{item.category} · {item.count} questions</Meta><Title>{item.title}</Title></View></View><Button secondary label={`Open ${item.title}`} onPress={()=>nav.navigate('Detail',{id:item.id})}/></Card>)}
