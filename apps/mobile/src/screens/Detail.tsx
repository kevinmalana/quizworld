...
export function Detail({route,navigation}:NativeStackScreenProps<Routes,'Detail'>){
 const [pack,setPack]=useState<Pack|undefined>(route.params.pack);const [loading,setLoading]=useState(!pack);const [error,setError]=useState('');const [retry,setRetry]=useState(0);const [confirm,setConfirm]=useState(false);const [checking,setChecking]=useState(false);const store=usePractice();
 ...
 const start=async(mode:Mode)=>{
  if(!pack||checking)return;
  setChecking(true);setError('');
  try {
   await checkPublicAccess(pack);
   const saved=await store.update(s=>startSession(s,pack,mode,randomUUID(),Date.now()));
   if(saved)navigation.navigate('Study');
  } catch(e){setError(e instanceof Error?e.message:'Could not check access.');}
