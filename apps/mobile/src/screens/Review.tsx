...
export function Review(){
 ...
 {[...due,...later].map(item=><Card key={item.key}><Body>{item.source==='public'?'Saved public question · open review to check current access':item.question.text}</Body><Meta>{item.title} · {item.successes?'Correct on retry':'Missed in practice'} · {item.dueAt<=Date.now()?'Due now':`Due ${new Date(item.dueAt).toLocaleString()}`}</Meta>{removing===item.key?<><Meta>Remove this question from review? Your completed session history is kept.</Meta><Button label="Confirm removal" disabled={busy} onPress={()=>{void update(s=>({...s,reviews:s.reviews.filter(r=>r.key!==item.key)})).then(ok=>{if(ok)setRemoving(null);});}}/><Button label="Keep question" secondary onPress={()=>setRemoving(null)}/></>:<Button secondary label={`Remove from review: ${item.source==='public'?item.title:item.question.text}`} disabled={busy} onPress={()=>setRemoving(item.key)}/>}</Card>)}
 <Meta>Review uses saved question versions. Public content requires a connection to recheck access before starting a new review. Connect and sync personal practice in Account for cross-device review. Official results and XP are never uploaded here.</Meta></Page>;
}
